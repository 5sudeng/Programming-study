...
class Heap:
    ...
    def __init__(self, capacity: int):
        ...
        self.heap_size = 0
        self.harr = [-sys.maxsize-1 for i in range(capacity)]
    ...
    def maxHeapify(self, i: int):
        ...
        if largest != i:
            self.harr[i], self.harr[largest] = swap(self.harr[i], self.harr[largest])
            self.maxHeapify(largest)

    def buildMaxHeap(self):
        for i in range(math.floor(self.heap_size/2)-1, -1, -1):
            self.maxHeapify(i)

    def heapsort(self):
        self.buildMaxHeap()
        for i in range(self.heap_size-1, 0, -1):
            self.harr[0], self.harr[i] = swap(self.harr[0], self.harr[i])
            self.heap_size -= 1
            self.maxHeapify(0)
    ...
    def maxHeapIncreaseKey(self, i: int, key: int):
        ...
        while (i > 0) and (self.harr[self.parent(i)] < self.harr[i]):
            self.harr[i], self.harr[self.parent(i)] = swap(self.harr[i], self.harr[self.parent(i)])
            i = self.parent(i)
